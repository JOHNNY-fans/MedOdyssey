...
async def get_ans(que, task_name, base_dir, model_name, result_file_name):
    que_input = que['input']
    que_type = que['type']
    que_id = que['id']
    ...
    match task_name:
        ...
        case 'en_counting':
            question_str = que_input
            que_answer = json.loads(que_answer)
        case 'zh_counting_general':
            question_str = que_input
            que_answer = json.loads(que_answer)
        case 'en_counting_general':
            question_str = que_input
            que_answer = json.loads(que_answer)
        case 'zh_counting_max':
            question_str = que_input
            que_answer = json.loads(que_answer)
        case 'en_counting_max':
            question_str = que_input
            que_answer = json.loads(que_answer)
        case _:
...
async def main():
    ...
    VERSION = 'v1'
    RESULT_FILE_NAME = 'query_compare_result/' + TASK_NAME + '_' + MODEL_NAME + '_' + VERSION + '.jsonl'
    
    # qa
    qa_list = []
    qa_dir = os.path.join(BASE_DIR, TASK_NAME, 'input_data_moonshot-v1.jsonl')
